...
export class LivenessAnalyzer implements Analyzer<LivenessInput> {
  ...
  buildPrompt(input: LivenessInput): { system: string; user: string } {
    ...
    lines.push('');
    // Stale and multi-source paths are what this analyzer exists to name, so
    // they are the rows a cut must keep.
    const { rows, omitted } = capPromptRows(input.paths, {
      priority: (p) => p.stale || p.multiSource,
    });
    for (const p of rows) {
