...
- The panel uses `signalk-nearlcrews-ui` for its root, theme contract, layout, fields, controls, feedback, and action bar. Fresh profiles use Auto and follow the Signal K host. Explicit preferences use `signalk-nearlcrews-ui.theme.v1`, and the retired `orc-theme` key is intentionally ignored. Keep plugin-specific presentation in CSS Modules, use the shared tokens, and do not hardcode colors.
