...
- TypeScript 6.0.3 and ESM are the development baseline. The published plugin supports Node 22.18 or newer; source development requires Node 22.22.2+, Node 24.15+, or Node 26. The primary CI gate uses Node 26, and Signal K plugin CI tests Node 22 and 24. esbuild bundles the backend to `dist/index.js` with target `node22`; webpack and esbuild-loader bundle the React panel to `public/remoteEntry.js`. Vitest covers unit tests. Biome owns formatting and baseline lint rules, ESLint adds typed and React Hooks rules, Markdownlint checks documentation, and cspell checks prose. `npm run type-check` runs four chained TypeScript passes: backend (`tsconfig.json`), tests (`tsconfig.tests.json`), panel (`tsconfig.panel.json`), and browser tooling (`tsconfig.tools.json`).
...
- Panel code is split: data and effects live in `src/configpanel/hooks/` (`useConfig`, `useStatus`, `useOpenRouterModels`, `useSaveLifecycle`), presentational pieces live in `src/configpanel/components/` (`AnalyzerRow`, `StatusBlock`, `OpenRouterSection`, `HistorySection`, `PromptDrawer`, and `IntegerInput`), and shared pure helpers live in `utils.ts`, `fireOutcome.ts`, and `scheduleOptions.ts`. Relative age formatting comes from the shared UI package. `PluginConfigurationPanel.tsx` composes them.
