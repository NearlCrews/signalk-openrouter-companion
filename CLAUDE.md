...
- Shared infra in `src/core/`: `logger`, `buffer`, `budget`, `engineDetector`, `batteryMonitor`, `cronScheduler`, `triggerRouter`, `openrouter`, `history` (read-only provider contract), `questdb`, `influxdb`, `http` (`fetchWithTimeout`), `publisher`, `discovery`, `skNode` (including `asTreeMap` and `readBankSnapshot`), `emitter` (TypedEmitter base for batteryMonitor and engineDetector), `readings` (shared rolling per-source map helpers: `fuseMin`, `fuseMax`, and `evictStaleSpan`), `format`, `paths`, `triggers` (including `buildTriggers` and `manualPutCtx`), `cfg` (including `clampPositiveInt`, `clampMin`, `clampRange`, `finiteOr`, and `resolveSystemPrompt`), and `api` (REST routes and PluginRuntime).
...
- Panel code is split: data and effects live in `src/configpanel/hooks/` (`useConfig`, `useStatus`, `useOpenRouterModels`, `useSaveLifecycle`), presentational pieces live in `src/configpanel/components/` (`AnalyzerRow`, `AnalyzerDrawer`, `StatusBlock`, `OpenRouterSection`, `HistorySection`, `PromptDrawer`, and `IntegerInput`), and shared pure helpers live in `utils.ts`, `fireOutcome.ts`, and `scheduleOptions.ts`. Relative age formatting comes from the shared UI package, always through the `RELATIVE_AGE_FORMAT` constant in `src/configpanel/relativeAge.ts` so a second age display cannot drift from the first. `PluginConfigurationPanel.tsx` composes them.
...
- `THIRD_PARTY_NOTICES.md` is GENERATED by `npm run licenses`, never hand-edited. `npm run package:check` re-verifies it with `--check`.
